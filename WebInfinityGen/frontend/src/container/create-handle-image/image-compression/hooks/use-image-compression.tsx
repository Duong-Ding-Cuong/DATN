...
export type ImageData = {
    ...
    processedImageSize?: number; // 🆕 Size in bytes
};
...
export const useImageCompression = () => {
    ...
    const calculateBase64Size = (base64String: string): number => {
        ...
        const byteSize = (base64Data.length * 3) / 4 - paddingCount;
    ...
    const callN8nAPI = async (imageFile: File): Promise<AIResponse> => {
        try {
            ...
            const response = await fetch(
                "http://localhost:5678/webhook-test/image-compression",
                {
                    method: "POST",
                    headers: {
    ...
    const handleProcessImage = async () => {
        ...
        try {
            ...
            // 🆕 Calculate processed image size
            const processedSize = response.processedImage
                ? calculateBase64Size(response.processedImage)
                : undefined;

            console.log("📊 Image sizes:", {
                originalSize: imageData.originalFile.size,
                processedSize,
                compression:
                    processedSize && imageData.originalFile.size
                        ? (
                              ((imageData.originalFile.size - processedSize) /
                                  imageData.originalFile.size) *
                              100
                          ).toFixed(2) + "%"
                        : "N/A",
            });
